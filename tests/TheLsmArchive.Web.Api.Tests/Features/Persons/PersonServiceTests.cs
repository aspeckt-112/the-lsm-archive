...
#pragma warning disable IDE0022
        await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            _personService.GetByTopicId(id, TestContext.Current.CancellationToken));
#pragma warning restore IDE0022
    }

    [Fact]
    public async Task GetByTopicId_WithNoAssociatedPersons_ReturnsEmptyList()
    {
        // Arrange
        TopicEntity topic = new() { Name = "Topic 1", NormalizedName = "topic1" };
        await InsertSingleInstanceOfEntityAsync(topic);

        // Act
        List<Person> people = await _personService.GetByTopicId(topic.Id, TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task GetByTopicId_WithAssociatedPersons_ReturnsPersonList()
    {
        // Arrange
        PersonEntity person1 = new() { Name = "Person A", NormalizedName = "persona" };
        PersonEntity person2 = new() { Name = "Person B", NormalizedName = "personb" };
        ...
        PersonTopicEntity pt1 = new() { PersonId = person1.Id, TopicId = topic.Id };
        PersonTopicEntity pt2 = new() { PersonId = person2.Id, TopicId = topic.Id };
